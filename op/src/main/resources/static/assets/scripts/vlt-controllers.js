	...
	VLTJS.contactForm = {
		init: function () {
			...
			contactForm.each(function () {
				...
				if (goReg && goReg.length) {
					goReg.on('click', function() {
						var btn = thisForm.find('.submit-btn')
						var regShow = thisForm.find('.register-show')
						var inputs1 = regShow.find('input')

						// 清空输入和验证状态
						thisForm.find('input').val('').removeClass('error')
				...
				thisForm.validate({
					...
					submitHandler: function (form) {
						...
						var url = ''
						var json = {}
						if (thisForm.find('.submit-btn').text() == '注册') {
							url = '/registUser'
							backFun =  function () {
								thisForm.find('.submit-btn').text('登陆')
								$('.go-register').text('去注册')
								thisForm.find('.register-show').hide()
								regShow.find('input').attr('required', false)
							}
						...
						formData.append('properties', JSON.stringify(json))
						$('.loading-div').show()
						$.ajax({
							type: 'POST',
							url: url,
							...
							success: function (data) {
								...
								if(data.sus!=null) {
									...
									errorMessage.text(data.error || '请求失败')
									errorMessage.fadeIn();
									setTimeout(function () {
