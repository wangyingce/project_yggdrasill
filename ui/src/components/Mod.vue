  ...
  <div class="content">
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        >删除node</el-button
        >
        <el-button size="small" type="primary" @click="updateNodeProperty()">更新属性</el-button>
      </span>
    </el-dialog>
    <el-dialog
            class="dialog-index"
    ...
    >
      <div>
        <el-row v-for="(v, i) in tempLinks" :key="i" :gutter="8">
          ...
          <el-col :span="2">
            <el-button size="mini" type="text" @click="delLink(i)">删除</el-button>
          </el-col>
        </el-row>
        <div>
          <el-button type="text" size="small" @click="addLink">新增连线</el-button>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
    ...
    methods: {
            ...
            this.$nextTick(() => {
              ...
              if(d.id!=null&&d.id!=undefined&&d.id!=""){
                axios.get('/updateNode',{
                  params: {id: d.id,properties: JSON.stringify(d.properties),},})
                        .then(function (response) {
                          if(response.status != 200 || response.data.error) {
                            vm.$message.error(response.data.error || response.statusText)
                            return
                          }
                          vm.$message.success("更新节点属性成功");
                          vm.reload();
                        }).catch(function (error) {
                  console.log(error);
                });
              }else{
                axios.get('/addNode'+ location.search,{
                  params: {type: d.type,properties: JSON.stringify(d.properties),},})
                        .then(function (response) {
                          if(response.status != 200 || response.data.error) {
                            vm.$message.error(response.data.error || response.statusText)
                            return
      ...
      open2() {
        ...
        axios.get('/showUserModelKg' + location.search)
                .then(function (response) {
                  ...
                  vm.tempLinks = vm.links = response.data.edges;
                  vm.visible2 = true;
                })
                .catch(function (error) {
                  console.log(error);
      ...
      ok2() {
        ...
        for (let i = 0; i < this.tempLinks.length; i++) {
          ...
          if (links.includes(it.source + it.target)) {
    ...
    mounted() {
      ...
      axios.get('/showUserModelKg' + location.search)
              .then(function (response) {
                ...
                vm.nodes=response.data.nodes;
                vm.links=response.data.edges;
